...
- Initialize repo if needed: `git init`; respect `.gitignore`.
- Commit in small, logical chunks with imperative summaries, e.g. `docs: align agent briefs`.
- Use scoped messages when helpful (`plot`, `build`, `docs`); include a short body when rationale isn’t obvious.
- Flow: `git add -A && git commit -m "scope: summary"`; avoid checking in `node_modules/`, `.venv/`, `.vercel/`.
